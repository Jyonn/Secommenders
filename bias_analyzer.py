...
def inspect_checkpoint(path, precision=4, include_model=True, as_json=False):
    ...
    if not matrix or any(len(row) != len(matrix) for row in matrix):
        raise ValueError(f'{parameter_name} must be a non-empty square matrix')
    ...
    names, catalog = _representation_names(config, len(matrix))
    ...
    print()
    print(_format_matrix('-- raw attention-logit bias --', matrix, labels, precision))
    print()
    print(_format_matrix('-- effective preference (row-centered) --', centered, labels, precision))
    print()
    print('-- strongest preference within each query row --')
